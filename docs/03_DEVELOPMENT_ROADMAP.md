...
- **Prasyarat untuk Fase 4 (Expense Management)** — Expense Management memakai pola posting yang identik, jadi `IJournalPostingService` harus sudah terbukti bekerja di modul AR/AP dulu sebelum direplikasi ke Expense.
...
- Tidak bergantung pada Fase 4 (Expense Management) — keduanya independen; Costing dikerjakan lebih dulu karena lebih mendesak untuk akurasi HPP/Utang Usaha yang sudah aktif dipakai sejak Fase 2.
...
## Fase 4 — Expense Management (Operational Expense / OPEX)
...
- Berada setelah Fase 2 (butuh pola posting jurnal `IJournalPostingService` yang sudah terbukti bekerja di AR/AP) dan setelah Fase 3 (Costing, dikerjakan lebih dulu karena lebih mendesak — lihat catatan di Fase 3), dan sebelum Fase 6 (Laporan Keuangan Formal / Cash & Bank Enhancement) karena laporan itu harus sudah mencakup data Expense supaya lengkap.
...
## Fase 5 — SupplierInvoice (AP Invoice mandiri)
...
- Trial Balance, Laba Rugi, Neraca, Cash Flow sebagai halaman baru di Finance module — mencakup data dari **semua** fase sebelumnya termasuk Expense Management (Fase 4).
...
├── Accounts Payable         (sudah ada, auto-posting sejak Fase 2, jadi entity mandiri di Fase 5)
├── Expense Management       (BARU — Fase 4)
│   ├── Expense Categories
│   ├── Expense Entry
...
- **Riwayat penomoran ulang**: `02_ACCOUNTING_MODULE_PROPOSAL.md` bagian 5 aslinya Fase 0-5 (tanpa Expense Management). Waktu Expense Management ditambahkan sebagai requirement baru, ia sempat disisipkan sebagai "Fase 3" (menggeser Costing/SupplierInvoice/Laporan Keuangan masing-masing jadi Fase 4/5/6). Setelah implementasi sungguhan berjalan (Fase 0→1→2 selesai), **Inventory Costing dikerjakan lebih dulu** daripada Expense Management karena lebih mendesak untuk akurasi HPP/Utang Usaha — sehingga di dokumen ini nomornya **ditukar**: Costing = Fase 3 (selesai), Expense Management = Fase 4 (belum dimulai). Isi kedua fase **identik** dengan rencana sebelumnya, hanya nomornya yang ditukar supaya konsisten dengan urutan implementasi sungguhan.
