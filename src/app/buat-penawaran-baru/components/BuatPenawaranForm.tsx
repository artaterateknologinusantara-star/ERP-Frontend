...
import CatatanPentingSection from './CatatanPentingSection';
import CatatanTambahanSection from './CatatanTambahanSection';
import type { CostingTab, PaymentTerm, NoteItem, QuotationStatus } from '@/types';
import { quotationService, mapTabsToBackend } from '@/services/quotation.service';

const initialTabs: CostingTab[] = [
  { id: 'tab-material-utama',     label: 'Material Utama',     sortOrder: 0, groups: [] },
  { id: 'tab-material-pendukung', label: 'Material Pendukung', sortOrder: 1, groups: [] },
  { id: 'tab-biaya-instalasi',    label: 'Biaya Instalasi',    sortOrder: 2, groups: [] },
  { id: 'tab-device-items',       label: 'Device Items',       sortOrder: 3, groups: [] },
];
...
export default function BuatPenawaranForm() {
  ...
  const [importantNotes, setImportantNotes] = useState<NoteItem[]>([]);
  ...
  const buildDto = () => {
    ...
    const notesStr = importantNotes.filter((n) => n.text).map((n, i) => `${i + 1}. ${n.text}`).join('\n');
    return {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {/* Page Header */}
      <div className="flex items-center justify-between flex-wrap gap-3 sticky-action-bar px-5 py-3 -mx-5 -mt-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="btn-primary" onClick={() => router.push('/riwayat-penawaran')}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        <div className="xl:col-span-2 space-y-5">
          ...
          />
          <CatatanPentingSection notes={importantNotes} onChange={setImportantNotes} />
